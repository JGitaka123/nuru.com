...
export default function AdminVerificationPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeading eyebrow="Operations" title="Verification queue" subtitle="Approve tenant, agent, and landlord identity submissions." />
      <AdminNav active="/admin/verification" />
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {items.map((user) => (
            <li key={user.id} className="rounded-2xl border border-ink-200 bg-surface p-5 shadow-card">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-serif text-lg text-ink-900">{user.name ?? "Unnamed user"}</p>
                ...
                <div className="flex gap-2">
                  <button disabled={busy === user.id} onClick={() => review(user, "VERIFIED")} className={`${btnBrand} disabled:opacity-50`}>Approve</button>
                  <button disabled={busy === user.id} onClick={() => review(user, "REJECTED")} className="rounded-xl border border-red-300 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-50 disabled:opacity-50">Reject</button>
                </div>
              </div>
            </li>
          ))}
